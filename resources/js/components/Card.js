...
class Card extends React.Component {
  ...
  setStageSize() {
    ...
    if ( window.screen.width > 768 && window.innerWidth > 768) {
      scaleX = 768 / this.state.stageWidth * 0.50
    } else {
      scaleX = window.screen.width / this.state.stageWidth * 0.56
    }
  ...
  setStageType(type) {
    ...
    var imageDraggable = (type == "card4x5") ? true : false
  ...
  setText() {
    ...
    if (event.target.name == "textTitle") {
      if (this.state.textTitle !== event.target.name) {
        // currentImage.textLine =  event.target.value.split("\n")
        currentImage.textTitle =  event.target.value
      }
    }
  ...
  countTextFirstLine(target) {
    if (target == "title") {
      return this.state.currentImage.textTitle.length
    }
  ...
  setCard() {
    ...
    maskHeight =  (this.state.stageHeight * 0.10).toFixed()
    ...
    maskRectangles.map((rect, i) => {
      ...
      if(rect.id == "bottommask") {
          rect.width = this.state.stageWidth
          rect.y = this.state.stageHeight - rect.height
          rect.height = rect.height
      }
  ...
  handleDragEnd(e) {
    ...
    } else if(e.currentTarget.attrs.id == "textTitle"){
      currentImage.textTitleX = e.currentTarget.attrs.x
      currentImage.textTitleY = e.currentTarget.attrs.y
    } else {
        ...
        <div key="current-div" className="row">
          <div className="col-sm-6 p-0 pt-2 rounded-bottom" style={stageTypeCss}>
              ...
              <li className="nav-item active">
                <a className="nav-link" id="item1-tab" data-toggle="tab" href="#item1" role="tab" aria-controls="item1" aria-selected="true" name="stageType" value="card4x5" onClick={() => this.setStageType("card4x5")}>カード4x5
                </a>
              </li>
            </ul>
                  ...
                  >
                    <Layer>
                     ...
                     <Text
                        id="textTitle"
                        key={"textTitle"}
                        ref={this.textTitleRef}
                        fontSize={Number(this.state.currentImage.fontSize)}
                        fontStyle={"bold"}
                        fontFamily={"メイリオ"}
                        text={this.state.currentImage.textTitle}
                        wrap="char"
                        lineHeight={1.4}
                        letterSpacing={2}
                        width={this.state.stageWidth}
          ...
          <div className="col-sm-6">
            <div className="card p-3 mt-2 bg-silver text-white">
              <div className="input-group mb-2">
                ...
                <input className="form-control" name="textTitle" value={this.state.currentImage.textTitle} onChange={this.setText} placeholder="タイトル表示させるテキストを入力" />
              </div>
